...
- When the configured provider declares image support, the triggering message's image attachments are validated, downloaded, and submitted to the model as image content; see [Image input](image-input.md). Without that flag, image attachments are ignored and behavior is unchanged.
...
- Register the model's input modalities from the provider definition: text only by default, plus image when `supportsImageInput` is set, so image content reaches the harness only for image-capable providers.
- Restore the native harness state for the logical session directly from durable SQLite entries.
- Supply the current normal message as the new prompt, or the formatted thread snapshot for a thread message. When image input is enabled and the triggering message carried images, attach those validated images to the prompt call and append a deterministic note stating the attachment count.
- Enable only `web_fetch`, the six GitHub custom tools when configured, the seven scoped memory tools, and the fixed-source `hsvai_graph_search` and `hsvai_graph_query` tools. Disable built-in write, edit, shell, filesystem-search, prompt templates, repository context, and all other agentic extensions. Skills discovery stays disabled (`noSkills`): when the selected persona declares curated bundled skills, pass exactly those directories as additional skill paths and enable the built-in `read` tool so PI lists the skills and loads their `SKILL.md` on demand (see [persona skills](persona-skills.md)); when the persona declares none, keep `read` disabled and the session unchanged.
...
- Register and allowlist `web_fetch`, token-gated GitHub tools, scoped memory tools, fixed-source HSVAI graph search, and the built-in `read` tool only when the selected persona declares bundled skills; disable every other built-in tool and build the system instruction from conversation kind and registered-tool metadata, including the Capability Gap Protocol.
