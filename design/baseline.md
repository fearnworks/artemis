...
- Let deployments enable image-attachment input for vision-capable models through provider configuration, while text-only deployments keep the previous behavior.
...
- Existing Ollama endpoint, model, and API key variables, plus an optional model config path and API key. A selected JSON definition owns provider identity, endpoint, model, context limits, optional explicit reasoning effort, reasoning support, PI compatibility flags, and optional image-input support.
...
The Discord adapter owns gateway connection, interaction registration, incoming-event normalization, typing indicators, outbound replies, and connection lifecycle logging. It exposes normalized events to the application rather than leaking Discord SDK objects into the conversation core. When the configured model declares [image input](image-input.md), the adapter also collects and validates the triggering message's image attachments before normalization.
...
10. Submit the current message, or the complete thread snapshot for a thread reply, to PI through the configured model provider. Include the stable conversation key, triggering author ID, and Discord message ID so memory tools can bind scope and provenance. When [image input](image-input.md) is enabled, submit the triggering message's validated image attachments as image content alongside the prompt.
...
- Invalid, oversized, or undownloadable image attachments when image input is enabled: fail the turn through the same generation-failure path. See [Image input](image-input.md).
